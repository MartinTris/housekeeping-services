...
const HousekeeperPersonalTrends = () => {
  ...
  const fetchTrends = async () => {
    try {
      const url = `${API_URL}/api/trends/housekeeper-trends?granularity=${granularity}`;
    ...
    if (granularity === "daily") {
      ...
      return days.map((day) => {
        ...
        if (selectedServiceType === "all") {
          const serviceTypeKeys = new Set(rawData.map((r) => r.service_type));

          serviceTypeKeys.forEach((key) => {
  ...
  const getColorForServiceType = (serviceType) => {
    ...
    const baseType = serviceType.toLowerCase();
  ...
  return (
    <div className="bg-white/90 backdrop-blur-sm p-4 sm:p-6 rounded-2xl shadow-lg mt-4 sm:mt-6">
      <h2 className="text-green-900 font-bold text-lg sm:text-xl mb-3 sm:mb-4">
        My Housekeeping Service Trends
      </h2>
...
export default HousekeeperPersonalTrends;