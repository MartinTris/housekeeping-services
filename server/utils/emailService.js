...
transporter.verify((error, success) => {
  if (error) {
    ...
    console.error('SMTP User:', process.env.BREVO_SMTP_USER);
    console.error('SMTP Key:', process.env.BREVO_SMTP_KEY ? 'Set' : 'Missing');
  } else {
...
async function sendVerificationEmail(email, token, firstName) {
  ...
  const mailOptions = {
    from: `"DLSU-D Housekeeping Services" <${process.env.SENDER_EMAIL}>`,
    to: email,
    subject: 'Verify Your Email Address - Housekeeping Services',
    html: `
      <!DOCTYPE html>
      <html>
      <head>
  ...
  try {
    ...
    console.error('   SMTP User being used:', process.env.BREVO_SMTP_USER);
    console.error('   SMTP Key present:', process.env.BREVO_SMTP_KEY ? 'Yes' : 'No');
    throw error;
  }
}
